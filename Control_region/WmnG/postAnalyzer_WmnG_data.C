...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
  {
    ...
    phoCand1   = getPhoCand(220,1.4442,1);
...
//new medium
  ...
  for(int p=0;p<nPho;p++)
    ...
    bool kinematic = uncorrectedPhoEt > phoPtCut  && fabs((*phoSCEta)[p])<phoEtaCut;
      bool photonId = (
         ((*phoHoverE)[p]                <  0.02197   ) &&
         ((*phoSigmaIEtaIEtaFull5x5)[p]  <  0.01015 ) &&
         ((*phohasPixelSeed)[p]              ==  0      ) &&
...
double postAnalyzer::FakeRatePt(Float_t phoPt, int syst_number)
{
  ...
  Float_t bin2[11] = {0.07529868,0.0779771,0.080763,0.0779608,0.0796352,0.0796462,0.0777209,0.0805415,0.0778359,0.0799271,0.0782949};
  Float_t bin3[11] = {0.06781591,0.0838512,0.0875113,0.0838365,0.0858884,0.0861928,0.0843523,0.0870061,0.084068,0.0873089,0.0836209};
